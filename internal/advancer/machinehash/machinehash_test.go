...
func (s *ValidateMachineHashSuite) TestItFailsWhenHttpEndpointIsInvalid() {
	err := ValidateMachineHash(
		...
		s.machineDir,
		s.appAddr,
		"://invalid-endpoint:8545",
	)
	s.ErrorContains(err, "failed to create ethclient")
}

func (s *ValidateMachineHashSuite) TestItFailsWhenCannotContactHttpEndpoint() {
	err := ValidateMachineHash(
		s.ctx,
		s.machineDir,
		s.appAddr,
	...
	s.ErrorContains(err, "failed to call contract method")
}

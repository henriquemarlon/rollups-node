...
func catchUp(ctx context.Context,
	...
	if uint64(len(inputs)) != processedInputs {
		errorMsg := fmt.Sprintf("processed inputs do not match: expected %d, got %d", len(inputs), processedInputs)
		logger.Error(errorMsg, "app", appAddress)

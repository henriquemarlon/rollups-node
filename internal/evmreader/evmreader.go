...
type Service struct {
	...
	epochLengthCache        map[string]uint64
	hasEnabledApps          bool
...
func (c *CreateInfo) LoadEnv() {
	...
	c.InputBoxAddress = common.HexToAddress(config.GetContractsInputBoxAddress()).String()
...
func Create(c *CreateInfo, s *Service) error {
	...
	return service.WithTimeout(c.MaxStartupTime, func() error {
		...
		inputSource, err := NewInputSourceAdapter(common.HexToAddress(c.InputBoxAddress), client)
		if err != nil {
			return err
		}
...
func (s *Service) SetupNodeConfig(
	...
	_, err := r.GetNodeConfig(ctx)
	if err == pgx.ErrNoRows {
		s.Logger.Debug("Initializing node config", "config", c)
		err = r.CreateNodeConfig(ctx, c)
		if err != nil {
			return err
		}
	} else if err == nil {
		s.Logger.Warn("Node was already configured. Using previous persistent config", "config", c)
	} else {
...
type EvmReaderRepository interface {
	...
	GetNodeConfig(context.Context) (*model.NodeConfig, error)
...
func (r *Service) Run(ctx context.Context, ready chan struct{}) error {
	...
	r.epochLengthCache = make(map[string]uint64)
	...
	for {
		...
		case header := <-headers:
			...
			for _, app := range runningApps {
				...
				// Get App EpochLength
				err = r.addAppEpochLengthIntoCache(aContracts)
				if err != nil {
					r.Logger.Error("Error adding epoch length into cache",
						"app", aContracts.application.IApplicationAddress,
						"error", err)
					continue
				}
